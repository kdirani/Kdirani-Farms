...
export function ExportMedicinesInvoicesButton() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!invoices || invoices.length === 0) {
        alert('لا توجد فواتير أدوية للتصدير');
        return;
      }
      ...
      XLSX.utils.book_append_sheet(workbook, invoicesWorksheet, 'فواتير الأدوية');
        ...
        for (const invoice of invoices) {
          ...
          if (settings.includeItems) {
            const { data: items } = await supabase
              ...
              `)
              .eq('medicine_invoice_id', invoice.id);

            if (items && items.length > 0) {
              const itemsData = items.map(item => {
                return {
                  'رقم الفاتورة': invoice.invoice_number,
                  'اسم الدواء': item.medicine?.medicine_name || '-',
              ...
              if (itemsData.length > 0) {
                ...
                XLSX.utils.book_append_sheet(workbook, itemsWorksheet, `عناصر فاتورة الأدوية ${invoice.invoice_number}`);
              }
            }
          }
          ...
          if (settings.includeExpenses) {
            ...
            if (expenses && expenses.length > 0) {
              ...
              if (expensesData.length > 0) {
                ...
                XLSX.utils.book_append_sheet(workbook, expensesWorksheet, `مصاريف فاتورة الأدوية ${invoice.invoice_number}`);
              }
            }
          }
        }
      ...
      const fileName = `فواتير_الأدوية_${new Date().toLocaleDateString('ar-SA').replace(/\//g, '-')}.xlsx`;
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>إعدادات تصدير فواتير الأدوية</DialogTitle>
        </DialogHeader>
